...
export default function MessageOptionsModal() {
  ...
  const { isOpen, selectedUserUid, selectedMessage, closeMessageOptionModal } =
  ...
  const reportUser = async () => {
    if (!user || !selectedUserUid || !selectedMessage) return;
    try {
    ...
    <Modal
      ...
      <div className="bg-grey py-3 px-5 text-background flex items-center justify-between">
        <div className="flex gap-3 items-center">
          <UserInfo user={selectedUserUid} />
        </div>
        <button onClick={closeMessageOptionModal}>
          <AiOutlineClose className="h-6 font-bold" />
        </button>
      </div>
